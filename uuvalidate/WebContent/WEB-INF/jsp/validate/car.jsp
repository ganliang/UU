			...
			<form
				...
				<table>
					<tr>
						...
						<td>状态:</td>
						<td><select name="userCarValidate">
								<c:if test="${userCarValidate==0}">
		...
		<table border="1px" width="100%">
			...
			<c:forEach items="${userlist}" var="user" varStatus="status">
				<tr style="height: 40px">
					...
					<td align="center"><c:choose>
							<c:when test="${user.userCarValidate==0 }">未审核</c:when>
							<c:when test="${user.userCarValidate==1 }">正在审核</c:when>
							<c:otherwise>审核通过</c:otherwise>
						</c:choose></td>
					<td align="center"><c:choose>
							<c:when test="${user.userCar!=null}">
								<a
									href="<%=request.getContextPath()%>/validate/showDescribeImages.do?userCar=${user.userCar}"
					...
					<td align="center"><c:choose>
							<c:when test="${user.userCarValidate==1}">
								<a onclick="return confimReject()"
									href="<%=request.getContextPath()%>/validate/validateReject.do?type=car&userId=${user.userId}&currentPage=${page.currentPage}&userCarValidate=${user.userCarValidate}&userName=${userName}&userTel=${userTel}">拒绝</a>
							</c:when>
					...
					<td align="center"><c:choose>
							<c:when test="${user.userCarValidate==1}">
								<a onclick="return confimAdopt()"
									href="<%=request.getContextPath()%>/validate/validateAdopt.do?type=car&userId=${user.userId}&currentPage=${page.currentPage}&userCarValidate=${user.userCarValidate}&userName=${userName}&userTel=${userTel}">通过</a>
							</c:when>
	...
	<div style="height: 20%; margin-top: 5px;">
		...
		<c:choose>
			<c:when test="${page.currentPage!=1}">
				<a
					href="<%=request.getContextPath()%>/validate/validateSearch.do?currentPage=${page.currentPage-1}&type=car&userCarValidate=${userCarValidate}&userName=${userName}&userTel=${userTel}">上一页</a>
			</c:when>
		...
		<c:choose>
			<c:when test="${page.currentPage!=page.totalPage }">
				<a
					href="<%=request.getContextPath()%>/validate/validateSearch.do?currentPage=${page.currentPage+1}&type=car&userCarValidate=${userCarValidate}&userName=${userName}&userTel=${userTel}">下一页</a>
			</c:when>
